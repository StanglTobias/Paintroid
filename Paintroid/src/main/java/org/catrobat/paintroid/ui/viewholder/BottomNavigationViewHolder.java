...
public class BottomNavigationViewHolder implements MainActivityContracts.BottomNavigationViewHolder {
	...
	public BottomNavigationViewHolder(View layout, Context context) {
		this.layout = layout;
		this.activeColor = getColor(context, R.color.pocketpaint_bottom_navigation_selected);
		this.inactiveColor = getColor(context, R.color.pocketpaint_bottom_navigation_not_selected);
	...
	public void setLandscapeStyle(Context context) {
		...
		for (int i = 0; i < menu.size(); i++) {
			...
			if (item.getId() != bottomNavigationView.getSelectedItemId()) {
				icon.setColorFilter(inactiveColor, PorterDuff.Mode.MULTIPLY);
	...
	public void setSelectedItem(int newItemId) {
		BottomNavigationItemView item = bottomNavigationMenuView.findViewById(bottomNavigationView.getSelectedItemId());
		ImageView icon = item.findViewById(R.id.icon);
		TextView text = item.findViewById(R.id.title);
		icon.setColorFilter(inactiveColor, PorterDuff.Mode.MULTIPLY);
		text.setTextColor(inactiveColor);

		item = bottomNavigationMenuView.findViewById(newItemId);
		icon = item.findViewById(R.id.icon);
		text = item.findViewById(R.id.title);
		icon.setColorFilter(activeColor, PorterDuff.Mode.MULTIPLY);
